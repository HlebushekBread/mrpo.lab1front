...
export class OrderEditComponent implements OnInit{
  ...
  readonly totalPrice = computed(() => {
    return this.orderProducts().reduce((acc, item) => {
      const productInfo = this.products().find((p: Product) => p.article === item.product.article);
  ...
  submitForm(formDataRaw: any) {
    const productsDto = this.orderProducts().map(item => ({
